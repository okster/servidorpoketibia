...
local talkUser = NPCHANDLER_CONVBEHAVIOR == CONVERSATION_DEFAULT and 0 or cid
        ...
        if (msgcontains(msg, 'clan') or msgcontains(msg, 'Clan')) then   --alterado v1.7
           if getPlayerClanName(cid) == 'Naturia' then       --alterado v1.7
        ...
        elseif (msgcontains(msg, 'yes') or msgcontains(msg, 'Yes')) and talkState[talkUser] == 2 then
           ...
           setPlayerClan(cid, 'Naturia')  --alterado v1.7
           talkState[talkUser] = 0
           
           
         elseif (msgcontains(msg, 'leave') or msgcontains(msg, 'Leave')) then  --alterado v1.7  \/
           if getPlayerClanName(cid) ~= 'Naturia' then 
        ...
        elseif (msgcontains(msg, 'shiny') or msgcontains(msg, 'Shiny')) then    --alterado v1.7
           if getPlayerClanName(cid) ~= 'Naturia' then    --alterado v1.7
              ...
              selfSay("So you want to transform your Vileplume in a Shiny Vileplume? It will cost 500k!", cid)
        ...
        elseif (msgcontains(msg, 'yes') or msgcontains(msg, 'Yes')) and talkState[talkUser] == 4 then
	       ...
	       if getItemAttribute(pb, "poke") ~= "Vileplume" then
	          selfSay("Put a Vileplume's pokeball atleast +20 in main slot!", cid)
              talkState[talkUser] = 0
              return true
           end
              
           if not getItemAttribute(pb, "boost") or getItemAttribute(pb, "boost") < 20 then
              selfSay("Sorry, your Vileplume is not boosted +20!", cid)
              talkState[talkUser] = 0
              return true
           end
           ...
           if doPlayerRemoveMoney(cid, 50000000) == true then
              ...
              doItemSetAttribute(pb, "poke", "Shiny Vileplume")
		      doItemSetAttribute(pb, "description", "Contains a Shiny Vileplume.")
		      doItemEraseAttribute(pb, "boost")
		      doTransformItem(getPlayerSlotItem(cid, 7).uid, fotos["Shiny Vileplume"])
		      if useKpdoDlls then
