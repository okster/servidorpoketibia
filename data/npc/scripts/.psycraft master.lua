...
local talkUser = NPCHANDLER_CONVBEHAVIOR == CONVERSATION_DEFAULT and 0 or cid
        ...
        if (msgcontains(msg, 'clan') or msgcontains(msg, 'Clan')) then   --alterado v1.7
           if getPlayerClanName(cid) == 'Psycraft' then   --alterado v1.7
              selfSay("You already are of the clan psycraft!", cid)
        ...
        elseif (msgcontains(msg, 'yes') or msgcontains(msg, 'Yes')) and talkState[talkUser] == 2 then
           ...
           selfSay("OK then... Now you are a great member of de clan psycraft!", cid)
           setPlayerClan(cid, 'Psycraft')   --alterado v1.7
           talkState[talkUser] = 0
           
           
        elseif (msgcontains(msg, 'leave') or msgcontains(msg, 'Leave')) then  --alterado v1.7  \/
           if getPlayerClanName(cid) ~= 'Psycraft' then 
        ...
        elseif (msgcontains(msg, 'shiny') or msgcontains(msg, 'Shiny')) then    --alterado v1.7
           if getPlayerClanName(cid) ~= 'Psycraft' then    --alterado v1.7
              selfSay("You aren't of the clan Psycraft! Get out of here!", cid)
              return true
           else   
              selfSay("So you want to transform your Hypno in a Shiny Hypno? It will cost 500k!", cid)
        ...
        elseif (msgcontains(msg, 'yes') or msgcontains(msg, 'Yes')) and talkState[talkUser] == 4 then
	       ...
	       if getItemAttribute(pb, "poke") ~= "Hypno" then
	          selfSay("Put a Hypno's pokeball atleast +20 in main slot!", cid)
              talkState[talkUser] = 0
              return true
           end
              
           if not getItemAttribute(pb, "boost") or getItemAttribute(pb, "boost") < 20 then
              selfSay("Sorry, your Hypno is not boosted +20!", cid)
              talkState[talkUser] = 0
              return true
           end
           ...
           if doPlayerRemoveMoney(cid, 50000000) == true then
              ...
              doItemSetAttribute(pb, "poke", "Shiny Hypno")
		      doItemSetAttribute(pb, "description", "Contains a Shiny Hypno.")
		      doItemEraseAttribute(pb, "boost")
		      doTransformItem(getPlayerSlotItem(cid, 7).uid, fotos["Shiny Hypno"])
		      if useKpdoDlls then
