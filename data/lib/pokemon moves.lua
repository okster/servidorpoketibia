...
elseif spell == "Leaf Storm" or tonumber(spell) == 73 then
	...
	addEvent(doDanoWithProtect, 150, cid, GRASSDAMAGE, getThingPosWithDebug(cid), grassarea, -min, -max, 245)
...
addEvent(doMoveInArea2, 2*450, cid, 241, flames0, FIREDAMAGE, min, max, spell)
for i = 0, 3 do
...
elseif spell == "Rollout" then
    ...
    if isSleeping(cid) then return true end
       ...
       doDanoWithProtect(cid, ROCKDAMAGE, getThingPosWithDebug(cid), splash, -min, -max, 127)
    end
...
local eff = getSubName(cid, target) == "Shiny Onix" and 39 or 39  --alterado v1.6.1
...
elseif spell == "Shockquake" then

local eff = getSubName(cid, target) == "Shiny Onix" and 152 or 152  --alterado v1.6.1
 
local function doQuake(cid)
if not isCreature(cid) then return false end
if isSleeping(cid) and getPlayerStorageValue(cid, 3644587) >= 1 then return false end
if isWithFear(cid) and getPlayerStorageValue(cid, 3644587) >= 1 then return true end
   doMoveInArea2(cid, eff, confusion, ELECTRICDAMAGE, min, max, spell)
end
...
if isSleeping(cid) and getPlayerStorageValue(cid, 3644587) >= 1 then return false end
if isWithFear(cid) and getPlayerStorageValue(cid, 3644587) >= 1 then return true end
   doMoveInArea2(cid, eff, confusion, GRASSDAMAGE, min, max, spell)
end
...
elseif spell == "Steelquake" then

local eff = getSubName(cid, target) == "Shiny Onix" and 223 or 223  --alterado v1.6.1
 
local function doQuake(cid)
if not isCreature(cid) then return false end
if isSleeping(cid) and getPlayerStorageValue(cid, 3644587) >= 1 then return false end
if isWithFear(cid) and getPlayerStorageValue(cid, 3644587) >= 1 then return true end
   doMoveInArea2(cid, eff, confusion, STEELDAMAGE, min, max, spell)
end
...
elseif spell == "Inferno Sombrio" then

local eff = getSubName(cid, target) == "Shiny Onix" and 55 or 55  --alterado v1.6.1
...
elseif spell == "Waterquake" then

local eff = getSubName(cid, target) == "Shiny Onix" and 89 or 89  --alterado v1.6.1
...
elseif spell == "Shadowquake" then

local eff = getSubName(cid, target) == "Black Articuno" and 152 or 2  --alterado v1.6.1
 
local function doQuake(cid)
if not isCreature(cid) then return false end
if isSleeping(cid) and getPlayerStorageValue(cid, 3644587) >= 1 then return false end
if isWithFear(cid) and getPlayerStorageValue(cid, 3644587) >= 1 then return true end
   doMoveInArea2(cid, eff, confusion, ghostDmg, min, max, spell)
end
...
elseif spell == "Destruction Earth" then

       local rounds = math.random(4, 7)
       rounds = rounds + math.floor(getPokemonLevel(cid) / 35)
       local eff = {239, 100, 239, 100, 157}
       local area = {psy1, psy2, psy3, psy4, psy5}
       ...
       addEvent(setPlayerStorageValue, 4*400, cid, 3644587, -1)
       for i = 0, 4 do
           addEvent(doMoveInArea2, i*400, cid, eff[i+1], area[i+1], GROUNDDAMAGE, min, max, spell, ret)
       end
...
addEvent(doFall, 450, cid)
addEvent(doDanoWithProtect, 1400, cid, ROCKDAMAGE, getThingPosWithDebug(cid), waterarea, -min, -max, 127)
